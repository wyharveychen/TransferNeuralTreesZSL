classdef NDFLayers < matlab.mixin.Copyable
    ...
    methods
        ...
        function layer = NDFLayers(indim,label_list)
            ...
            layer.D      = layer.G; 
            layer.m      = layer.G;
            %layer.G      = mat2cell(zeros(layer.tree_num,1),ones(layer.tree_num,1));
        ...
        function next_in = Forward(layer,in) %in: input, next_in: layer output (next layer input)             
             ...
             next_in  = mean(cat(3,layer.p{:}),3);
             layer.next_in = next_in;
        end
        function next_in = ForwardOneTree(layer,in,tree_id) %in: input, next_in: layer output (next layer input)             
        ...
        %% For Backward
        function prev_derr_merged = Backward(layer,derr) %derr: derr/ds                          
        ...
        function prev_derr_merged = BackwardOneTree(layer,derr,tree_id)
             layer.yid  =  layer.LabelMapping(derr);
        ...
        function Update(layer)
            if(strcmp(layer.optimizer,'Adam'))
                ...
                layer.theta = cellfun(@(w_th,m,G) (w_th - layer.learn_rate*(m/(1-adam_beta1))./(sqrt(G/(1-adam_beta2))+1e-8)), layer.theta, layer.m, layer.G,'UniformOutput',0);    
            elseif(strcmp(layer.optimizer,'RMSprop'))
                layer.G = cellfun(@(g,G) (layer.ada_decay_rate*G + (1-layer.ada_decay_rate)*g.^2),layer.grad ,layer.G,'UniformOutput',0 ); 
                layer.theta = cellfun(@(w_th,grad,G) (w_th - layer.learn_rate*grad./(sqrt(G)+1e-8)), layer.theta, layer.grad, layer.G,'UniformOutput',0);    
            elseif(strcmp(layer.optimizer,'adadelta'))
                layer.G = cellfun(@(g,G) (layer.ada_decay_rate*G + (1-layer.ada_decay_rate)*g.^2),layer.grad ,layer.G,'UniformOutput',0 ); 
                ...
                layer.theta = cellfun(@(w_th,x) (w_th + layer.learn_rate.*x), layer.theta, X,'UniformOutput',0);              
            elseif(strcmp(layer.optimizer,'adagrad'))
                layer.G = cellfun(@(g,G) (G + g.^2),layer.grad ,layer.G,'UniformOutput',0 ); 
                layer.theta = cellfun(@(w_th,grad,G) (w_th - layer.learn_rate*grad./(sqrt(G)+1e-8)), layer.theta, layer.grad, layer.G,'UniformOutput',0);             
            else
                layer.theta = cellfun(@(w_th,grad) (w_th - layer.learn_rate*grad), layer.theta, layer.grad,'UniformOutput',0);             
            end
        ...
        function predicted_label = Predict(layer)
            [~,max_id] = max(layer.next_in,[],1);
            predicted_label = layer.label_list(max_id);
        end

        function RecordForestNorm(layer,derr)
        ...
        function ClearCache(layer)
            layer.in = []; layer.d = []; layer.mu = []; layer.p = []; layer.next_in = [];
